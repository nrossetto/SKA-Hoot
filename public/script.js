...
// Carregar configuração visual da sala
    ...
    try {
        ...
        if (data.sucesso) {
            ...
            if (data.logo_base64) {
                ...
                logoArea.innerHTML = `<img src="${data.logo_base64}" style="max-width: 200px; margin-bottom: 20px;">`;
            }
        }
    } catch(e) {
...
function mostrarPergunta(pergunta, botoes) {
    ...
    if (pergunta.imagem_base64) {
        imgDiv.innerHTML = `<img src="${pergunta.imagem_base64}" alt="Imagem da pergunta">`;
    } else {
        imgDiv.innerHTML = '';
    }
    
    // Atualizar textos dos botões
    document.getElementById('botaoA').innerHTML = botoes.A;
    document.getElementById('botaoB').innerHTML = botoes.B;
    document.getElementById('botaoC').innerHTML = botoes.C;
    document.getElementById('botaoD').innerHTML = botoes.D;
...
function mostrarRanking() {
    ...
    if (ranking.length === 0) {
        lista.innerHTML = '<div>Aguardando respostas...</div>';
    } else {
...
// Bloquear letras no campo de código
